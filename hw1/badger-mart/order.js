...
function calculateSubtotal() {
	...
	for (const item of ITEMS) {
		let price_id = item + "-price";
		let price = parseFloat(document.getElementById(price_id).innerText);
		let quantity_id = item + "-quantity";
		let quantity = parseInt(document.getElementById(quantity_id).value);
		cost += price * quantity;
	}
	return cost;
}
...
function calculateSalesTax() {
	...
	const tax = subtotal * getSalesTaxRateForState(state).toFixed(2);
	return tax;
}
